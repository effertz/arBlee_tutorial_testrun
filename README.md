...
This tutorial was created using a MacBook and iPhone, but you can also use your Android mobile phone and / or a Windows Desktop or Laptop. However, to create an app for iPhone, you need to install [Xcode](https://developer.apple.com/xcode/) on Apple hardware. Doing it on a Windows machine is possible, but complicated and not part of this tutorial.
...
1. First add a plane manager to your session origin: Select *AR Session Origin* in *Hierarchy* tab, go to the *Inspector* tab, press *Add Component*, search for *AR Plane Manager* and add it to the *AR Session Origin*.
2. Create a plane prefab: Right click in *Hierarchy* tab and select *XR* → *AR Default Plane*. Create a *Prefab* folder in your project: Select *Assets* folder in *Project* tab, right click, select *Create* → *Folder* and rename it to *Prefab*. Move the *AR Default Plane* from the *Hierarchy* tab to the newly created *Prefab* folder. Delete the *AR Default Plane* in the *Hierarchy* tab.
3. Assign the plane prefab to the plane manager: Select the *AR Session Origin* in *Hierarchy* tab, go to the *Inspector* tab, open the *Plane Prefab* dropdown in the *AR Plane Manager*, select the *Assets* tab and select the *AR Default Plane* prefab.
...
After making sure that this part is working, remove the *AR Default Plane* prefab from the *AR Plane Manager*: You can do that in the *Inspector* tab. Click on the *Plane Prefab* dropdown and select *None*.
...
1. Add the marker asset to your Unity project: create a new folder in the *Project* tab and name it *Textures*. Now drag and drop the indicator asset into this folder. Select the newly added asset, enable *Alpha is Transparency* in the *Inspector* tab and press *Apply*. 
2. Create a material for the new asset: Create a new folder in the *Project* tab and name it *Materials*. Select it, right click and select *Create* → *Material* and rename it to *Indicator_mat*. While the new material is still selected, go to the *Indicator* tab, change the *Shader* to *Unlit* → *Transparent* and set the *Texture* to marker texture previously added.
3. Create an object for the marker: 
    1. Right click in *Hierarchy* tab, select *Create Empty* to create an empty game object and rename it to *PlacementIndicator*. Go to the *Indicator* tab and set the *Position* to (0,0,0). 
    2. Go back to the *Project* tab and create a plane object under *PlacementIndicator*: Right click and select *3D Object* → *Plane*. While the *Plane* is selected, go to the *Indicator* tab and
    ...
    4. Now, drag the *PlacementIndicator* material on the *Plane* object in the *Scene* tab in the center of the Unity Editor.
...
1. Create a new C# script: First create a *Scripts* folder under *Assets* in the *Hierarchy* tab. Then right click on the folder, select *Create* → *C# Script* and rename it directly to *PlacementIndicator*.
2. Attach the C# script to the *PlacementIndicator* game object: To do that, select the *PlacementIndicator* game object in the *Hierarchy* tab, go to the *Inspector* tab, press *Add Component*, search for the *Placement Indicator* C# script and select it.
3. Add logic to the *PlacementIndicator* C# script: Double click on the *PlacementIndicator* script in the *Hierarchy* tab to open it in *Visual Studio*. Replace the existing code with:
``` C#
...
5. Connect the *Plane* game object to the script: select the *PlacementIndicator* game object in the *Project* tab, go to *Inspector* tab, open the *Indicator Icon* dropdown and select *Plane*.
6. Add *AR Raycast Manager* to the *AR Session Origin*: Select *AR Session Origin* in the *Project* tab, go to the *Inspector* tab, click *Add Component*, search for *AR Raycast Manager* and select it.
...
1. Add logic to the *PlacementIndicator* C# script: double click on the *PlacementIndicator* script in the *Hierarchy* tab to open it in *Visual Studio*. Replace the *UpdateGamePlacementPose* method with the following code:
``` C#
void UpdateGamePlacementPose() {
    if (aRRaycastManager != null && Camera.current != null) {
        // shoot a raycast from the center of the screen
...
1. Create a new game object to be placed at the indicator location: Right click on the *Hierarchy* tab, press *Create Empty*, rename the object to *CubeObject* and set its *Position* to (0,0,0). Now, select the newly created object, right click, press *3D Object* → *Cube*. Set the cube's *Position* to (0,0.05,0) and *Scale* to (0.1, 0.1, 0.1). Delete it’s *Box Collider*, it’s not needed. 
2. Add logic to spawn the *Cube* to the C# script: double click on the *PlacementIndicator* script in the *Hierarchy* tab to open it in *Visual Studio*. Replace the existing code with: 
``` C#
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.XR.ARFoundation;
...
4. Connect the CubeObject object with the script: select the PlacementIndicator game object in the Project tab, go to Inspector tab, open the Object To Place Prefab dropdown and select CubeObject.
...
3. Import the character to Unity: Go to *Unity Editor* and create a *Characters* folder under *Assets* in the *Projects* tab. Drag and drop the downloaded character to the newly created folder. Now drag and drop the character from the *Characters* folder in the *Project* tab to the *Hierarchy* tab. The character will appear in the *Scene* tab. If you zoom in on the character in the *Scene* tab, you will notice that the textures are not displayed. Let’s fix that: Select your character in the *Project* tab, go to the *Inspector* tab, select *Materials*, click on *Extract Textures…* and select the *Textures* folder. While extracting Unity will warn you about normal map and ask you, if it should fix it: press *Fix now*. If you check your character in the *Scene* tab again, you will notice that the textures are displayed correctly.
4. Your character might be a bit dark, the *Directional Light* (the sun) might be pointing in the wrong direction. To change this: Select *Directional Light* in the *Hierarchy* tab, go to the *Inspector* tab and change the *Y*-value of the *Rotation* to adapt the light direction.
...
1. Attach the character to the *PlacementIndicator* script: select the *PlacementIndicator* game object in the *Project* tab, go to *Inspector* tab, open the *Object To Place* dropdown and select your character. You can also delete the *CubeObject* now. We will not need it anymore.
2. Change the size of the character: Select your character in the *Characters* folder in the *Project* tab, go to the *Inspector* tab, select *Model*, change the value for *Scale Factor* to 0.4 and press *Apply*.
...
2. Import the animation to Unity: Drag and drop the downloaded animation to the *Characters* folder in *Project* tab in the *Unity Editor*. Select *Assets*, right click and select *Create* → *Animation Controller*. Rename the animation controller to *CharacterAnimationController*. Select your character in the *Hierarchy* tab, go to the *Inspector* tab and add an *Animator* component. Open the *Controller* dropdown in the *Animator* component and select the *CharacterAnimationController*. Next, double click on the *CharacterAnimationController* in the *Project* tab. It will open the *Animator* tab for this controller. Right click on the squared *Base Layer* area, create a new state, using the *Empty* state. Rename this state to *Idle*. Open the dropdown for the *Motion* field in the *Inspector* tab and select *Idle*. To repeat this animation endlessly, select the animation in the *Project* tab, go to *Inspector* tab, enable *Loop Time* and press *Apply*.

Build your application again and check how your character behaves.

YouTube tutorial: [Import Mixamo Characters and Animation](https://www.youtube.com/watch?v=lFQHhGtFLog)
